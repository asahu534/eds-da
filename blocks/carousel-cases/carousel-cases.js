...
export default async function decorate(block) {
  ...
  // The first row may be a single-cell headline ("Our work in action") that sits
  // in a left column beside the carousel (source layout). Detect it — a row whose
  // only child div holds a heading and nothing else (no image cell) — pull it out
  // of the slide list and render it as the block's leading headline.
  ...
  if (allRows.length) {
    ...
    if (firstCols.length === 1 && heading && !firstRow.querySelector('picture, img')) {
      headlineEl = document.createElement('div');
      headlineEl.classList.add('carousel-cases-headline');
      headlineEl.append(heading);
      firstRow.remove();
    }
  }
  ...
  // Place the headline (if any) as the left column, before the carousel container.
  if (headlineEl) {
    block.classList.add('has-headline');
    block.prepend(headlineEl);
  }
