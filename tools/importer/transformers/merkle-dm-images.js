 ...
 * Converts DM/Scene7 <img> tags into anchors so the DM URL round-trips through
 * markdown intact (raw <img src> would not). A companion auto-block in
 * scripts/scripts.js rebuilds responsive <picture> markup at render time.
 *
 * Source uses Scene7 IS/Image URLs, e.g.:
 *   https://assets.merkle.com/is/image/merkle/...:Image-Card-L?op_sharpen=1&...
 * (20 such URLs detected in migration-work/metadata.json .images.mapping)
 *
 * Runs in afterTransform ONLY: block parsers run between the hooks and extract
 * <img> references into block cells (cards/carousel). Rewriting imgs to anchors
 * in beforeTransform would leave parsers with empty image cells.
 */

// ---- Begin canonical helpers (copied from dm-scene7-helpers.js — keep byte-identical) ----
